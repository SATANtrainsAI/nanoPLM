...
def print0(*args, **kwargs):
    """
    Print only on the main process in a distributed run, or normally if not distributed.
    """
...
class ProteinTokenizer:
    ...
    def pad_sequences(self, sequences, padding_value=None, block_size=None):
        ...
        return [
            list(seq)[:block_size] +
            [padding_value] * max(0, block_size - len(seq))
            for seq in sequences
        ]

# -----------------------------------------------------------------------
# Custom IterableDataset for FASTA
# -----------------------------------------------------------------------
class ProteinIterableDataset(IterableDataset):
    ...
    def process_sequence(self, sequence):
        ...
        if self.block_size is not None:
            ...
            input_ids += [self.tokenizer.token_dict[self.tokenizer.pad_token]] * max(
                0, self.block_size - len(input_ids)
            )
...
def adjust_logits_for_repetition(logits, generated_seq, rep_penalty=4):
    """
    If the last token is repeated consecutively `rep_penalty` times,
    set its logit to -∞ to block it.
    """
    logits = logits.clone()
    B = logits.size(0)
    for i in range(B):
...
def adjust_logits_for_ngram_blocking(logits, generated_seq, n=3):
    """
    If adding a token would form an n-gram that already appeared, block it.
    """
    logits = logits.clone()
    B = logits.size(0)
    for i in range(B):
...
parser.add_argument(
    ...
    help='Initialize from scratch or resume.'
)
...
parser.add_argument(
    ...
    default='bfloat16', help='Data type for model weights.'
)
...
parser.add_argument(
    ...
    help='Iterations between evaluations.'
)
...
dl = DataLoader(
    train_dataset, batch_size=args.batch_size, collate_fn=collate_fn_with_tokenizer,
    num_workers=0, pin_memory=True, drop_last=True
)
val_dl = DataLoader(
    val_dataset, batch_size=args.batch_size, collate_fn=collate_fn_with_tokenizer,
    num_workers=0, pin_memory=True, drop_last=True
)
...
if args.init_from == "resume":
    ...
    if missing_keys:
        ...
        print("Likely new modality layers, randomly initialized.")
    if unexpected_keys:
...
# Quick test batch to ensure DataLoader works
